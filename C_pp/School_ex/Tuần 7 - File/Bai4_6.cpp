...
int main() {
    ...
    // Output
    sort(mathang, mathang + n, sapxep);	// Sắp xếp thứ tự các mặt hàng
    for (int i = 0; i < n; i++) {
        outpf << mathang[i].ma_mathang << " " << mathang[i].ten_mathang << " " << mathang[i].so_luong << " " << mathang[i].gia_ban << "\n";
    }

    /* Cách 2 - Tuy nhiên cần khai báo thongtin mathang[n] thay vì thongtin mathang[50] bởi sẽ chạy hết mảng mathang 
